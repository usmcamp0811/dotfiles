...
with lib.fmf; {
  ...
  microvm = {
    ...
    shares = [
      ...
      }
      {
        proto = "virtiofs";
        tag = "rw-store";
        source = "/persist/vm-stores/vm-k8s-worker/nix-store";
        mountPoint = "/nix/.rw-store";
      }
      {
        proto = "virtiofs";
        tag = "vault-agent";
        source = "/persist/system/var/lib/vault/vm-k8s-worker";
        mountPoint = "/var/lib/vault/vm-k8s-worker";
      }
      {
        proto = "virtiofs";
        tag = "k8s-data";
  ...
  fmf = {
    ...
    user = {
      name = "admin";
      fullName = "K8s Worker Administrator";
    ...
    services = {
      vault-agent = {
        ...
        settings.vault = {
          ...
          role-id = "/var/lib/vault/vm-k8s-worker/role-id";
          secret-id = "/var/lib/vault/vm-k8s-worker/secret-id";
        };
      };
      ...
      k3s = {
        ...
        config = {
          node-name = "vm-k8s-worker";
        };
      };
    };
  };

  # Open necessary ports for k8s worker
  networking.firewall = {
    enable = true;
    allowedTCPPorts = [
      10250 # Kubelet
