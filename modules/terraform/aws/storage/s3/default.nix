...
let cfg = config.aws.storage.s3buckets;
in {
  ...
  config = mkIf cfg.enable {
    ...
    # Manage S3 buckets
    resource.aws_s3_bucket = builtins.listToAttrs (map (bucket: {
      name = bucket;
      value = {
        bucket = bucket;
        tags = cfg.tags;
    ...
    # Add bucket policies
    resource.aws_s3_bucket_policy = builtins.listToAttrs (map (bucket: {
      name = "${bucket}_policy";
      value = {
        bucket = bucket;
