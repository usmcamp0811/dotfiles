...
with lib;
with lib.campground;
let cfg = config.campground.apps.k9s;
in {
  options.campground.apps.k9s = with types; {
    ...
    vault-path = mkOpt str "secret/campground/k0s"
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.copyKUBECONFIG = {
      ...
      serviceConfig = {
        ...
        User = "root";
        ExecStart =
          "${pkgs.bash}/bin/bash -c 'mkdir -p /etc/k8s/ && cp /tmp/detsys-vault/kubeconfig /etc/k8s/config && chgrp k8s /etc/k8s/config'";
      };
          ...
          files = {
            "kubeconfig" = {
              text = ''
                ...
                {{ .Data.admin_kubeconfig }}
                {{ else }}
                {{ .Data.data.admin_kubeconfig }}
                {{ end }}
                {{ end }}
              '';
