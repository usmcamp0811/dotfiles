...
with lib;
with lib.fmf; let
  cfg = config.fmf.apps.k9s;
in {
  options.fmf.apps.k9s = with types; {
  ...
  config = mkIf cfg.enable {
    ...
    # systemd.services.copyKUBECONFIG = mkIf config.fmf.services.vault-agent.enable {
    ...
    fmf.services.vault-agent.services.copyKUBECONFIG = {
      settings = {
