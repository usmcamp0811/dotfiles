...
let
  ...
  gen-server = pkgs.writeShellScriptBin "generate-server-ovpn" ''
    ...
    SERVER_NAME=$1
    COMMON_NAME="''${SERVER_NAME}.client.${cfg.domain-name}"
    ...
    cat $CA_CERT > /var/lib/vault/ovpn/ca.crt
    cat $SERVER_CERT > /var/lib/vault/ovpn/server.crt
    cat $SERVER_KEY > /var/lib/vault/ovpn/server.key
# Fix permissions
