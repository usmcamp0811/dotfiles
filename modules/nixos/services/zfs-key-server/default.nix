...
in {
  options.fmf.services.zfs-key-server = with types; {
    ...
    # Allow null to disable keepalived usage.
    lan-ip = mkOpt (nullOr str) "10.8.0.55" "IP to use for the LAN Instance (null disables keepalived).";
  ...
  config = mkIf cfg.enable {
    # Only enable keepalived if lan-ip is set.
    fmf.services.keepalived = mkIf (cfg.lan-ip != null) {
      enable = true;
      instances = {
        "zfs-key-server" = {
