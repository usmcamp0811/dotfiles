...
with lib;
with lib.fmf; let
  cfg = config.fmf.services.postgresql;
in {
  options.fmf.services.postgresql = with types; {
  ...
  config = mkIf cfg.enable {
    ...
    fmf.services.prometheus.additionalCollectors = ["postgres"];
    ...
    fmf.services.vault-agent.services.set-postgres-passwords = {
      settings = {
