...
with lib;
with lib.campground;
let cfg = config.campground.services.user-secrets;
in {
  options.campground.services.user-secrets = with types; {
    ...
    vault-path = mkOpt str "secret/campground/users"
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services = lib.mapAttrs' (user: secrets: {
      ...
      value = {
        ...
        serviceConfig = {
          ExecStart =
            "${pkgs.bash}/bin/bash -c 'rm -rf /var/lib/vault/users/${user} && mkdir -p /var/lib/vault/users/${user} && chown ${user} /var/lib/vault/users/${user} && chmod 0700 /var/lib/vault/users/${user} && echo Move files && for secret in ${
