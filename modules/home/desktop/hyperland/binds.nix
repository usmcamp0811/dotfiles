      ...
      settings = {
        bind =
          [
            ...
            "$SUPER, L, exec, ${getExe config.programs.swaylock.package} --grace 0 --fade-in 0"
            ...
            # ░▀▀▀░▀▀▀░▀░▀░▀▀▀░▀▀▀░▀░▀░▀▀▀░▀░▀░▀▀▀░░▀░
            # Pictures
            ", Print, exec, file=\"/home/${config.campground.user.name}/Pictures/screenshots/$(date +'%Y%m%d_%H%M%S.png')\" && ${getExe pkgs.grim} \"$file\" && ${getExe pkgs.libnotify} --icon \"$file\" 'Screenshot Saved'"
            "SHIFT, Print, exec, file=\"/home/${config.campground.user.name})/Pictures/screenshots/$(date +'%Y%m%d_%H%M%S.png')\" && ${getExe pkgs.grim} -g \"$(slurp)\" \"$file\" && ${getExe pkgs.libnotify} --icon \"$file\" 'Screenshot Saved'"
            ...
            # Screen recording
            "SUPER_CTRLALT, Print, exec, ${getExe pkgs.campground.record_screen} screen"
            "SUPER_CTRLALTSHIFT, Print, exec, ${getExe pkgs.campground.record_screen} area"
