      ...
      settings = {
        bind =
          [
            ...
            # ░▀▀▀░▀▀▀░▀░▀░▀▀▀░▀▀▀░▀░▀░▀▀▀░▀░▀░▀▀▀░░▀░
            # Pictures
            # ", Print, exec, file=\"/home/${config.campground.user.name}/Pictures/screenshots/$(date +'%Y%m%d_%H%M%S.png')\" && ${getExe nixpkgs-wayland.packages.${system}.grim} \"$file\" && ${getExe pkgs.libnotify} --icon \"$file\" 'Screenshot Saved'"
            # "SHIFT, Print, exec, file=\"/home/${config.campground.user.name})/Pictures/screenshots/$(date +'%Y%m%d_%H%M%S.png')\" && ${getExe nixpkgs-wayland.packages.${system}.grim} -g \"$(slurp)\" \"$file\" && ${getExe pkgs.libn otify} --icon \"$file\" 'Screenshot Saved'"
