...
let
  ...
  opencodeAnthropicMaxFixPlugin =
    "op-anthropic-auth@${opencodeAnthropicMaxFixVersion}";

  opencodeAnthropicMaxFixScript =
    pkgs.writeShellScript "opencode-anthropic-max-fix.sh" ''
      set -euo pipefail
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.user.services.opencode-anthropic-max-fix =
      mkIf (cfg.enableOpencode && cfg.enableOpencodeAnthropicAuthPlugin) {
    ...
    systemd.user.paths.opencode-anthropic-max-fix =
      mkIf (cfg.enableOpencode && cfg.enableOpencodeAnthropicAuthPlugin) {
        Unit = {
