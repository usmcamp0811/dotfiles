...
in {
  ...
  config = mkIf cfg.enable {
    home.packages = [pkgs.protonmail-bridge pkgs.pass-secret-service ];
    ...
    systemd.user.services.protonmail-bridge = {
      ...
      Service = {
        Restart = "always";
        Environment = "PATH=${pkgs.pass-secret-service}/bin:${pkgs.protonmail-bridge}/bin:/run/current-system/sw/bin";
